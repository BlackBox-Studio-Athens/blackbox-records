...
| `storefront-catalog`   | `closed` | shopper-facing content query, catalog projection, slug tooling, cards/detail, and non-checkout store routes      | browser-safe catalog and route-projection surfaces       |
| `store-cart`           | `closed` | `apps/web/src/lib/store-cart.ts`, `apps/web/src/lib/money.ts`, StoreCart button and drawer                       | `@/lib/store-cart` plus cart UI surfaces                 |
...
| `commerce-domain`      | `closed` | backend commerce IDs, quantity value objects, and repository port contracts                                      | backend commerce repository SPI                          |
