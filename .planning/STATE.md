...
milestone: v1.3
milestone_name: Checkout Launch Readiness
current_phase: 13
...
**Current focus:** Phase 13 - stripe-dynamic-payment-methods-policy

## Current Position

Phase: 13 (stripe-dynamic-payment-methods-policy) - ACTIVE
Plan: 1 of 1
Current Phase: 13
Current Phase Name: stripe-dynamic-payment-methods-policy
Total Phases: 18
Current Plan: 1
...
Hardening handoff. Phase 12 boundary hardening is complete. Stripe sandbox UAT is available; production go-live remains
blocked on live Stripe access, final domain wiring, production webhook configuration, production Worker/D1 configuration,
and final approval. Phase 13 isolates the dynamic payment methods policy/configuration work. Phase 14 Shiplemon-powered
non-Greece shipping is post-MVP and should not block the active checkout launch readiness scope unless explicitly
reactivated.

GSD v1.42.3 operating note: this repo stays in flat planning mode and disables GSD worktree isolation for Codex
...
explicit phase or plan arguments for GSD commands; the current human focus is Phase 13 Stripe dynamic payment methods
policy, while Phase 14 is post-MVP.
...
- The no-account cart expansion workstream has landed for the v1 launch scope. Native commerce is now multi-item through
  StoreCart, CartDraft, CartLine, and CartQuantity, and the project will not return to the earlier single-item checkout
  scope.

- Current unpaid-session policy: expired Checkout Sessions transition CheckoutOrder to `not_paid`; declined card attempts
  remain `pending_payment` while the Stripe Checkout Session stays open and only close after a verified non-paid signal
  such as session expiry. Do not add more granular declined/abandoned states without production or operator evidence.

- Dynamic payment methods policy is approved for planning only. Future Stripe Checkout should use a named Stripe Payment
  Method Configuration and allow only cards, Apple Pay, Google Pay, and Link; PayPal, Klarna, BNPL methods, and
  bank-debit style methods must never be shown. No code, Stripe CLI mutation, Cloudflare env change, or account-specific
  configuration ID has been introduced by this planning slice.

- Phase 14 added post-MVP Shiplemon non-Greece shipping planning. Future non-Greece checkout should use Worker-owned
  Shipping Quotes, explicit Shipping Package Profiles, and operator-confirmed Shiplemon Shipment creation after payment.
  Greece remains on the existing manual BOX NOW path, and no Shiplemon credentials, labels, shipment IDs, or raw provider
  payloads have been introduced by this planning slice. It must not block the active checkout launch readiness scope unless
  explicitly reactivated.

- Phases 15-18 add independent dependency/tooling adoption plans: Sharp asset QA, `robot3` for the app-shell player
...
| post-MVP | Non-Greece shipping should use Shiplemon through Worker-owned quotes and operator-confirmed post-payment shipment creation; BOX NOW remains Greece-only manual v1 unless explicitly reopened. | Deferred |
...
- Treat `.planning/phases/14-shiplemon-non-greece-shipping-integration/14-01-PLAN.md` as post-MVP and stale against the
  updated context unless Phase 14 is explicitly reactivated and replanned.

- Phase 16 is complete: the app-shell player session reducer is backed by `robot3@^1.2.0` behind the existing
  `reducePlayerSessionMachine` adapter, with no rendered UI, routing, overlay, checkout, or backend order-state changes.

- Phase 17 is complete: repo-authored slug generation and validation now flow through `apps/web/src/lib/slugs.ts`, backed
  by `@sindresorhus/slugify@^3.0.0`, with existing public slugs, content filenames, route aliases, StoreItem identities,
  D1 mappings, and Stripe mappings preserved.
...
- Shiplemon API integration is post-MVP planning only. Do not commit Shiplemon credentials, account IDs, raw provider
  payload dumps, label URLs, invoice URLs, or live shipment evidence. Do not treat it as a launch-readiness blocker unless
  Phase 14 is explicitly reactivated.
