# Phase 8: Webhook Orders And Inventory - Context
...
- **D-03:** Phase 8 defines one business-named Stripe reconciliation use case shared by verified webhook handling and trusted session-status retrieval.
- **D-04:** Shared reconciliation logic can normalize observable checkout state for browser UX, but only verified webhooks may perform authoritative paid transitions or inventory decrement.
...
- Reuse the same backend normalization logic for trusted session-status reads and webhook-triggered reconciliation so order state does not drift across code paths.
