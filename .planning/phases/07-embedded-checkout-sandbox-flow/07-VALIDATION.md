...
- 2026-04-25: Checkout return UI was added at `/store/[slug]/checkout/return/`. It reads `session_id` only as input to the Worker-owned `ReadCheckoutState` endpoint, renders app-owned paid/open/processing/expired/unknown states, and keeps retry/cart/item/store actions non-authoritative.
