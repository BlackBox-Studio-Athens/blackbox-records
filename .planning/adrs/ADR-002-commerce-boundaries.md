...
- **Return and retry pages** retrieve checkout status only through Worker-owned APIs; raw Stripe query params or raw Stripe identifiers are not the durable browser contract.
- **Stripe-to-app state normalization** is expressed as one backend-owned reconciliation use case shared by trusted session-status retrieval and verified webhook handling.
...
- Server routes are mandatory for checkout-session creation, trusted session-status retrieval, and webhook work.
