...
## Durable preview-context follow-up — UAT verification (2026-09-23)

Release run [35934003253](https://github.com/BlackBox-Studio-Athens/blackbox-records/actions/runs/35934003253) completed successfully for `1e0c6885cbec63be62b90e4f8f8af48b542dc883`. The release validation, UAT and PRD frontend builds, Chromium/Firefox preview gate, UAT Pages inspection, UAT Worker deployment, UAT Pages deployment, release identity check, and UAT smoke all passed. PRD deployment and catalog jobs were skipped; no PRD promotion occurred.

The PRD Distro record ID is not present in UAT. On the matching UAT copy, `2016` / `Band in the pit` / `Cassette` (`01M2DC34K83K3Y03CF6M2X7BZ5`), Chrome reached “Preview up to date.” Detail showed the description, purchase information, release date, and all three gallery images; each image had a non-zero natural width after scrolling into view. Listing rendered the 101-item Distro catalogue and the expected `2016` / `Band in the pit` / `Cassette` card. This distinction matches the intended design; no content change was needed.

For recovery, I temporarily made the Chrome tab offline and changed the view. The workspace showed “Preview could not update” and “Showing the last successful preview”; the previous page stayed available. After restoring the connection, Retry succeeded and produced a current preview. Returning to Detail again loaded all three gallery images. The compact Lattice Loader was visible beside “Updating preview” while an update was pending. I did not edit, save, or publish content.

The browser-control session could not reproduce a real hidden-tab transition: its newly opened second page remained in the background while the editor still reported `document.visibilityState === "visible"`, and the connected Chromium protocol did not expose a visibility override. Earlier built-editor browser acceptance covered short hidden-tab reuse and expired-context refresh in both Chromium and Firefox, and the release's Chromium/Firefox gates passed. The user's actual Firefox/Chrome Alt+Tab check on the deployed UAT record remains outstanding. PRD was not promoted.

## Firefox remains unresolved — 2026-09-24

The user reports that the matching UAT Distro record remains on “Updating preview” indefinitely in Firefox, without diagnostic details. This supersedes any inference that the durable-context fix resolved their Firefox failure. Task 7.6 remains open and the change is not archived.

Native Chrome extension access initially timed out during DOM snapshot capture. The DevTools fallback opened a separate unauthenticated browser, so it provided no staff acceptance evidence. Reconnecting through the user's Chrome extension and inspecting via its CDP capability succeeded. A fresh hide/reopen preview attempt reached “Preview up to date”; creation, frame, observed scripts, styles, and image responses returned HTTP 200. The existing Worker log correlated request `5233827f-c3b5-4324-a516-ad07a595814a`, generation 10, release `1e0c6885cbec63be62b90e4f8f8af48b542dc883`, and a 588 ms frame render. No captured Chrome console errors were present.

A bounded filtered Wrangler tail captured successful Chrome renders but no correlated failing Firefox attempt. The tail also reported keep-alive disconnects and reconnection attempts before it was stopped; absent events therefore cannot establish that Firefox never reached the Worker. Firefox console evidence and a foreground reproduction longer than 35 seconds were requested from the user. No new behavioral fix is justified by the current evidence; Firefox's indefinite loading cause remains unconfirmed. No content was edited, saved, or published.

## Confirmed Firefox Access-session diagnosis — 2026-09-24

The user's foreground retry produced request `42047b2b-32c4-4969-8b81-2003c74a32e4`, release `1e0c6885cbec63be62b90e4f8f8af48b542dc883`, generation 1, displayed generation 0, and a frame-stage timeout. This establishes that the thirty-second timeout runs; the earlier indefinite-loading description did not identify a separate confirmed timer defect.

Using the Chrome GPT extension, Compute → Observability returned the matching `preview_browser_failure` at 10:42:52.580 GMT+3. There was no matching frame render. Firefox's console reported `frame-ancestors 'none'` at the preview hostname, alongside a `CF_Session` cookie warning. A bounded unauthenticated HTTP probe confirmed that the preview host redirects to `blackboxrecords.cloudflareaccess.com/cdn-cgi/access/login/preview-uat.blackboxrecordsathens.com`; that Cloudflare sign-in response has title “Sign in · Cloudflare Access” and CSP `frame-ancestors 'none'`. Our actual rendered preview policy allows the exact staff origin instead. The user then signed into the preview hostname in a separate Firefox tab and confirmed “Preview now loads.” This controlled change confirms the missing preview-host Access session as the cause of this failure. The earlier durable-context fix did not address this authentication path.

The follow-up adds a bounded credentialed session check before mounting a replacement iframe, a separate-tab sign-in link on connection failure, explicit `access` diagnostics, and an authenticated session response readable only by the configured staff origin. The existing login CSP, Access verification, context ownership and iframe sandbox remain enforced. Regression coverage models the non-embeddable login redirect and retry with previous-frame retention. Full final validation and UAT deployment of this follow-up are pending.

The full unit command passed, including all three preview-host tests. Standalone Chromium and Firefox editor suites passed the new Access regression and the existing workspace scenarios. The final editor run `2026-09-24T08-08-49-449Z-18000` also passed every phase, but its overall evidence was invalidated because OpenSpec documentation changed during that run. A fresh serialized full validation, editor validation, and canonical CMS build is required against the settled tree before push. The targeted change and main content-publishing specification both passed strict OpenSpec validation.
