...
- [x] 7.7 Verify missing preview-host Access sessions fail before iframe mounting, offer separate-tab sign-in and scoped retry, retain the previous preview, and report the access phase. Cover the non-embeddable login redirect in both browsers and validate exact staff-origin CORS and authentication.
- [ ] 7.8 Consolidate UAT Staff and Preview under the existing Staff Access application with eager cookie redirects, align the Worker audience, and verify a fresh Staff login loads Preview without a separate sign-in. Preserve the Google allowlist, isolated preview origin, context ownership, and separate PRD release gate.
