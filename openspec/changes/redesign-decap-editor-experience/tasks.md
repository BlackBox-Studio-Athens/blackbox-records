...
- [ ] 4.7 Replace the editor-visible Artist Slug field with non-required hidden Decap storage while keeping the saved Astro field required, use Decap's native `{{slug}}` filename template, and register one `artists`-only `preSave` generator before initialization through the existing shared slug helper; generate only when blank and never regenerate a nonblank slug after title edits.
- [ ] 4.8 Preserve every committed Artist slug, filename, public route, and Release reference; keep explicit overrides source-only and add round-trip, format, and collision coverage for those identities.
...
- [ ] 5.7 Replace blanket input box-model and link/button foreground rules with semantic scopes; keep real text controls styled while resetting internal `role="combobox"` inputs so every select and relation widget retains correct native geometry.
- [ ] 5.8 Ensure primary workflow actions and standalone icon controls are at least 44 by 44 CSS pixels with accessible names and visible focus; require WCAG 2.2 AA contrast for actionable default, hover, focus, and active states, keep disabled states legible and distinct, and verify Artist, Store Item, image, Release, navigation, validation, and preview tasks need neither hover nor desktop width.
- [ ] 5.9 Use the existing horizontal BlackBox wordmark as a dark mark on the light authenticated header, keep Contents text-only by hiding its native document SVG through the stable `a[href="#/"]` route selector, and add no generated-class or observer dependency.
- [ ] 5.10 Bind the standalone `cms:dev` Astro process to `127.0.0.1` so the page origin matches generated local site, display, logo, proxy, and smoke URLs without changing hosted behavior.
...
- [ ] 6.4 Add focused tests for Artist slug generation and preservation, absence of an editor-visible slug field, native filename configuration, composite-control CSS boundaries, action contrast selectors, header wordmark/icon behavior, and standalone local host consistency.
