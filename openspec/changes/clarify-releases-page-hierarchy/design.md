...
Three visual directions were reviewed. Option 1 is selected: a dominant latest-release region and a smaller artwork-backed Upcoming rail share one asymmetric desktop row, then `Our Releases` begins below. The current outlined action group remains part of the Feature Wall: the standalone Listen control with its indicator, View Release, and the commerce link returned by `getReleaseCommerceLink`.
...
- Make `Upcoming` and `Our Releases` separate, scannable editorial sections.
...
Keep the Feature Wall article focused on its artwork and copy. Move the existing selected-upcoming markup into a sibling `Upcoming` section with its own `h2`; render its title as the nested release heading. On wide screens, place the two sibling regions in the selected Option 1 asymmetric row, with the Feature Wall owning the larger main column and Upcoming owning the smaller right-hand rail. Add an `Our Releases` section and `h2` around the existing `remainingReleaseEntries` grid below that row. Source and keyboard order are Feature Wall, Upcoming, then Our Releases.
...
The Feature Wall remains unchanged apart from the layout wrapper needed to place its sibling rail. Upcoming becomes a restrained vertical editorial panel using its existing linked Astro image, status label, title, artist, semantic full date, optional summary, and formats. Its art and type remain clearly smaller than the Feature Wall, and it gains no new Listen, preorder, or Store action. At narrow widths, the row becomes one column in source order; Upcoming is not converted into a horizontal scroller or overlay.

Our Releases reuses `ReleaseCard` with its existing `framed-artwork` variant in the current three, two, and one-column responsive grid. This produces a quieter archive without a new card component, preserves the whole-card detail link and embedded-player trigger, and avoids a second metadata implementation.
...
No fixed content height or horizontal scroller is added. Long titles, summaries, metadata, and action labels must wrap within their grid column. Existing `.releases-latest-feature__actions`, action-link, and standalone Listen-control behavior is reused rather than replaced. Motion stays limited to existing image and focus transitions and continues to respect reduced motion.

### Verify the rendered hierarchy, not only the build

Browser Use must cover direct and shell-managed `/releases/` navigation at desktop and 320 to 390 CSS pixel widths. For current content it must confirm the selected asymmetric desktop composition, Caregivers in the Feature Wall, Disintegration in Upcoming, Anarchotribal under Our Releases, one occurrence per role, semantic heading order, and the current ordered Listen / View Release / resolved commerce action group. It must also confirm usable focus and links, release-overlay continuity, Listen/player behavior and indicator, the helper-owned commerce label and destination, stacked narrow reflow, no horizontal overflow, and a clean console.
