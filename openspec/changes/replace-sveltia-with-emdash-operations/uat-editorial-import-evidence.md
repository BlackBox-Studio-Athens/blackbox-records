...
## Frozen UAT import rehearsal — 2026-09-15

The sole editor confirmed that editorial writes were paused. From the existing source tree, the final UAT plan contains 129 records and 152 image paths, SHA-256 `ecf6f9abbd603063b9ce76c928d3f5eebfefeb1f007755019805771dc83d2bd8`. The native importer ran through WebStorm with an ordinary, application-scoped Cloudflare Access CLI login. It reused the existing `applyCmsImport` implementation without widening API permissions or changing its no-overwrite rule.

The run completed at `2026-09-15T06:26:59.014Z`: 433 bounded HTTP requests, including 152 deduplicating media submissions; zero new records and zero new media. Every source record and image passed the importer's content, reference, dimensions and checksum checks. Before/after D1 reads were exactly equal for all 142 rows in the native editorial tables, including revisions and the four newer acceptance drafts. Commerce comparison retained all 107 Store Item, Stock, provider mapping and offer rows, and all 518 CheckoutOrder rows, without changed prices, balances or historical links. The import transport never addresses commerce mutation routes or PRD.

The run reused the earlier successful Local repeat-import pilot and today's account headroom (120,920 D1 reads, 15,170 writes and 6,383 Worker requests observed before today's bounded backup/rehearsal work). It imposed a 600-request hard limit; no retries, reset, new storage resource or plan upgrade occurred. The original writer remained frozen; the importer did not overwrite CMS edits or publish drafts. This completes task 10.3's UAT rehearsal, not PRD cutover or retirement of the old writer.

Private evidence remains in `.codex-artifacts/emdash-m1/frozen-import-{preflight,before,after,rehearsal}.json`. The before/after captures contain private operational data and are not committed.
