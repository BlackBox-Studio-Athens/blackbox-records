...
The Store Distro category SHALL initially present every populated group as a complete Grid and SHALL offer a responsive BlackBox 3D Coverflow by explicit visitor choice when the group contains more than six Store Items, or when a group containing two or more Store Items is explicitly selected through enhanced format navigation, and the required client platform support is available. Selecting a format alone MUST NOT activate Coverflow. Each eligible group retains its own view controls; no page-wide Coverflow state is required.

#### Scenario: Large group is eligible

- **WHEN** `/store/distro/` has JavaScript and `transform-style: preserve-3d` support, no text or artist filter is active, a populated group contains more than six classified Store Items, and the visitor chooses Coverflow
...
- **WHEN** `/store/distro/` has JavaScript and `transform-style: preserve-3d` support, no text or artist filter is active, enhanced format navigation selects a group containing two to six Store Items, and the visitor explicitly chooses Coverflow
...
Each eligible Store Distro group MUST occupy exactly one of `preview`, `catalog`, or `search-results` mode, and active text or artist filtering MUST NOT coexist with a collapsed preview. Initial rendering uses catalog mode. Format selection returns to Grid, retaining search-results mode when text or artist filters remain active.

#### Scenario: Visitor requests the full group

- **WHEN** the visitor activates `Grid` from `preview` mode
- **THEN** that group enters `catalog` mode and exposes every classified Store Item exactly once in its existing canonical order within a continuous responsive grid
- **AND** the previously active Store Item receives focus, a subtle selected border and tonal surface, and an instant nearest-block scroll that makes it immediately recognizable below the fixed site header
- **AND** Grid is selected in the Grid then Coverflow view controls
...
- **WHEN** the visitor activates `Coverflow` while no text or artist filter is active
...
- **AND** Coverflow is selected in the Grid then Coverflow view controls.

#### Scenario: Distro search becomes active

- **WHEN** the normalized Store Distro search query becomes non-empty or an artist is selected
...
- **WHEN** all text and artist filters are cleared, or the Store Distro browse control cleans up on route exit
- **THEN** eligible groups leave `search-results` mode in `catalog` mode
- **AND** clearing text/artist filters preserves the current format in Grid, while route cleanup restores the full server catalogue
- **AND** neither action automatically activates Coverflow.
...
- **THEN** pre-cache snapshot sanitization restores the server-authored complete Grid, original order, and control contents while removing selected and temporary transition state
- **AND** the restored document paints directly as a Grid.
...
- **THEN** the complete Grid remains visible before and after controller readiness
- **AND** Coverflow becomes available only as an explicit second view.

#### Scenario: Visitor prefers reduced motion

- **WHEN** `prefers-reduced-motion: reduce` matches
- **THEN** Previous, Next, `Grid`, and `Coverflow` reach the same final states without Coverflow movement, shared-element animation, or stagger
...
The enhancement MUST reuse the same server-rendered Store Distro card nodes, without a fixed six-item grid row boundary, MUST NOT create a second catalog or commerce projection, and MUST remain within the fixed Store Distro performance budgets.

#### Scenario: Preview and catalog share content

- **WHEN** an eligible group moves between `preview`, `catalog`, and `search-results`
- **THEN** the same server-rendered Store Item card nodes change presentation in place
...
- **AND** only the front and visibly exposed side covers are promised as pointer targets while all positioned links, up to six, remain keyboard-operable.
...
- **WHEN** representative direct-load, shell-entry, and narrow-viewport checks exercise the revised default and existing Coverflow interactions
...
- **AND** the full Grid remains usable while controller readiness is delayed, with no pre-ready disclosure queue
...
- **AND** both supported and unsupported clients begin with the complete Grid
- **AND** existing project Core Web Vitals budgets remain policy, with no new exhaustive performance matrix unless the focused checks reveal a regression.
...
- **AND** the Grid choice offers return to the complete catalogue with an accessible name and a target at least 44 CSS pixels high
...
- **AND** catalog mode retains Grid then Coverflow choices while search-results mode retains filtering ownership and only Grid presentation.

#### Scenario: Disclosure motion runs

- **WHEN** motion is permitted and the visitor first explicitly activates an eligible preview
...
- **AND WHEN** the visitor activates `Grid`
