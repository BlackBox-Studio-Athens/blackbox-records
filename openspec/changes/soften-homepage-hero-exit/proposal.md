...
- Crossfade only the media layer from opacity `1` to `0.12` over 240 milliseconds when the existing coarse scrolled class changes state; never hide or remove it while Home remains active.
- Fade an absolute `#050505` veil inside that same media layer from opacity `0` to `0.5` on the identical curve, making the settled image read as near-black texture without adding markup or another fixed element.
...
- Add no markup, second fixed element, per-scroll property, transform, keyframe, parallax, filter, blur, blend mode, `will-change`, dependency, asset, or JavaScript state.
