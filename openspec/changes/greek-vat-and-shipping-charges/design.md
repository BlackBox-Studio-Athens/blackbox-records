...
### 3. Reuse runtime catalog ownership and hosted Checkout

Reconciled 2026-09-17: local monetary/packing implementation and provisional synthetic-packing UAT proof already exist. Remaining work is account, measured packing, fiscal-provider and full hosted acceptance. Items now owns explicit setup/price commands, runtime catalog data lives in D1, and normal code releases do not regenerate catalog projections or seed inventory. Use existing bound Product/default-Price checks and explicit replacement commands; repository recovery inputs are not routine catalog authority. Keep Orders read-only, Content responsible for approved purchase wording, and Stock responsible for physical/online quantities. Code corrections use `pnpm validate` and relevant editor checks; no new tax or fulfillment implementation is implied by this reconciliation.
