...
At the original final-tree capture, tasks 2.11, 3.11, 4.11, 5.10, 6.13, 7.7, 8.6, and 8.12 were open: Distro missed the literal frame gate at the runner cadence floor, Store missed application-work and long-task gates, and the repository-wide `pnpm check` was blocked by Prettier on an unrelated untracked catalog-discovery spec. The 2026-07-15 amendment below supersedes that closure state.

## 2026-07-15 closure amendment

The literal 16.7 ms frame-interval p95 is not a valid application discriminator in this runner: Home first and repeat both report 18.0–18.1 ms with sub-millisecond application work, no long task, and no layout work. Round two therefore calibrates frame-interval acceptance against that same-profile low-work control while retaining every application-work, layout, long-task, load, CLS, and rendered-UX gate. Distro eager/native is accepted only under that calibration: it remains at the 18.0 ms cadence floor with zero traversal layout activation and no long task.

Store is not accepted. Grouped containment, retained activation, and eager rendering were all rejected, and the original renderer remains unchanged. Its post-consolidation route shape must be remeasured before a future bounded remedy is proposed; that future work may not introduce pagination, virtualization, batching, static prices, or a commerce-authority change by implication. This amendment closes the decision boundary, not the Store performance miss.

Validation on 2026-07-15 passed through WebStorm MCP for `test:unit`, `check`, and `build`. `pnpm assets:check` passed with the three existing artist-source warnings, `pnpm cache:policy:check` passed, and the documented PRD-shaped build (`ASTRO_SITE_URL=https://blackbox-records-web.pages.dev`, `ASTRO_BASE_PATH=/`) passed Holding Page prepare/check. The initial Holding Page failure was correctly classified as attempting that PRD-only artifact preparation against a default UAT-base build; no production code change was warranted.
