...
- Build a minimal holding artifact in a separate manual-only GitHub Actions workflow and deploy it to an isolated Cloudflare Pages branch target through a named environment restricted to `main`, without a separate required-reviewer gate, preserving both GitHub Pages UAT and the full `blackbox-records-web.pages.dev` PRD readiness site.
- Make the apex hostname canonical through a staged custom-domain association that immediately targets the verified holding branch, redirect HTTP to HTTPS and `www` to the apex with exact-host rules, keep the temporary page out of search indexes, and hand verified domain/rollback evidence to `production-go-live-readiness`.
...
- `static-site-and-deployment`: Adds an isolated holding artifact and Pages branch deployment, custom-domain routing, and a verified rollback target for the production-go-live-owned cutover.
...
- Cloudflare Pages branch/custom-domain configuration, apex and `www` DNS/redirect behavior, TLS verification, and a redacted handoff of the verified rollback target.
...
- Full-site cutover, holding retirement, and post-launch cleanup are owned only by `production-go-live-readiness` or a child it explicitly creates.
