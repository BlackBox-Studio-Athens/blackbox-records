...
Closure decision on 2026-09-10: the user authorized accepting the retained image implementation with the subsequent containment change's documented timing exceptions. `README.md` records the matching source/build evidence and exact exceptions. This supersedes the original all-gates-pass archive condition below without changing the image behavior or the main performance budgets.
