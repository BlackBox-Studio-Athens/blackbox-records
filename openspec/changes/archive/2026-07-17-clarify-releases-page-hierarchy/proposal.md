...
- Place `Our Releases` directly beneath the Feature Wall in the wide main column while Upcoming can continue through the right-hand rail, eliminating catalog whitespace without changing source order.
- Give all release artwork the existing restrained card hover/focus motion, strengthen the two temporal labels, simplify release dividers, and normalize the `Our Releases` heading scale.
...
- Browser verification for direct and shell-managed `/releases/` at desktop and mobile widths, including the compact two-row asymmetric layout, shared artwork motion, semantic order, existing action controls, focus, player triggers, overlays, and horizontal reflow.
