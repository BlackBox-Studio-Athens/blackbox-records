...
See [proposal](proposal.md) and the 2026-09-14 discussion. Low traffic removes the case for cache/distributed infrastructure but not duplicate-write protection. Current checkout creates a random order ID per request and already handles stock holds, order-derived provider keys, uncertainty, and webhook recovery. StockChange accepts a delta without request identity; StockCount has `expectedRevision`. EmDash adds a durable journal for item/price operations. Reassess final implementations after that epic before adding fields.
...
The checkout UI generates a key before first submission and retains it with the attempt fingerprint in `sessionStorage`, outside StoreCart. Same-tab reload/network retry reuses it. Different cart quantities, shipping/consent inputs, or explicit new checkout create a new attempt; do not automatically rotate the key because a request failed. Known terminal completion clears browser attempt state. Return/cancel navigation must read actual state: cancellation alone does not prove a payable Session is expired. Independently intentional attempts in separate tabs remain independent purchases.
...
After EmDash and RFC 9457 acceptance, audit existing protection, add only missing fields/constraints and request support, migrate clients, then enforce. Run duplicate/concurrency/acknowledgement-loss/restart/provider-uncertainty tests with real Local D1 transactions. Budget any hosted test operations separately. Roll back compatible client/server releases without removing populated additive fields or losing pending holds; once enforcement is active, rollback must preserve keyed-request handling.
