## 1. Program Foundation

- [x] 1.1 Create the `site-performance-program` OpenSpec artifact graph with one `performance-program` capability.
- [x] 1.2 Record the native umbrella-and-independent-child model and reject a custom OpenSpec schema or duplicated child plans.
- [x] 1.3 Create the append-only `performance-report-log.md` contract and reserve stable `PERF-NNN` report identities.

## 2. Round One Registration And Closure

- [x] 2.1 Register `improve-site-runtime-performance` as round one without rewriting its proposal, design, specs, tasks, or implementation history.
- [x] 2.2 Confirm round one has complete artifacts, 60 of 60 completed tasks, and strict OpenSpec validation.
- [x] 2.3 Synchronize round-one deltas into baseline specs and archive the change at `../archive/2026-07-12-improve-site-runtime-performance/`.
- [x] 2.4 Register `PERF-001` as the round-one implementation closeout and link its canonical archived report.
- [x] 2.5 Store and register `PERF-002` as the independent post-round-one audit, including comparison limits and the accepted round-two backlog.

## 3. Round Two Planning And Implementation

- [x] 3.1 Create and strict-validate `improve-site-runtime-performance-round-two` with a complete proposal, design, delta specs, and detailed implementation tasks.
- [ ] 3.2 Apply the round-two child in its declared slice order, treating first traversal as the critical scroll experience and preserving the round-one Store request fix.
- [ ] 3.3 Run focused regression coverage, Browser Use rendered acceptance, fixed load/scroll profiles, hosted diagnostics, and the exact final-tree repository gates required by the child.
- [ ] 3.4 Append `PERF-003` with like-for-like before/after evidence, field-data confidence, residual misses, and any evidence-gated no-actions.
- [ ] 3.5 Strict-validate and archive round two after its report is registered and every child task is complete.

## 4. Future Performance Rounds

- [ ] 4.1 Add a future child only when a fresh report identifies a reproducible user-facing miss and a bounded remedy set.
- [ ] 4.2 Archive the completed predecessor before a new child modifies its baseline requirements.
- [ ] 4.3 Keep future implementation logic in the child; update only the epic register, sequencing tasks, and report ledger at program level.
- [ ] 4.4 Append a stable report entry after each implementation or formal post-implementation audit without overwriting prior evidence.

## 5. Program Closure

- [ ] 5.1 Close this epic only after explicit owner direction, with no active child and every completed implementation archived and reported.
- [ ] 5.2 Strict-validate the epic and full baseline spec set before archival.
