...
1. **Resolve through the existing content reader.** For `sourceKind === 'release'`, read the referenced release by `sourceId` through `@/lib/content-reader`, resolve its artist with existing catalog helpers, and build player data exactly as the release detail does. The same template must work with accepted-snapshot runtime reads, retained static builds, and authenticated CMS previews; do not import repository JSON or query editable CMS records from public rendering. Keep the data page-local; do not add provider URLs to StoreCart, checkout payloads, Stripe catalog projections, or backend contracts. A missing required release is a validation/render error, not a title-based fallback. Preserve preview sanitization: previewing the treatment must not activate third-party embeds.
...
No schema, database, or provider migration. Implement the source-backed actions in the current purchase hierarchy, verify accepted-snapshot and static release/Distro fixtures and direct/base-path routes, then exercise the existing player and overlays on desktop/mobile with Browser Use. Follow the paired renderer/CMS/Pages code release in `docs/catalog-promotion.md` when authorized; content publication is separate and does not rebuild code. Code rollback preserves the accepted content pointer. Apply the required Impeccable visual gates before UI implementation; no visual redesign is approved by this proposal.
