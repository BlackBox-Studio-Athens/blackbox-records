...
- [ ] 5.3 Under existing release authorization and a fresh Free-tier worksheet, deploy the canonical UAT candidate and smoke Overview, Website, Stock and Distro once each. Verify revision, functionality and built resource boundaries; retain existing release gates without duplicating the full PRD performance sample.
- [ ] 5.4 After separately authorized reviewed-SHA/candidate promotion and fresh budget, measure PRD with at most 16 route activations, one focus return and one three-GET control as specified in the design. Record every visit, revision, network/cache conditions, HTML/asset/API/usable-content timing and artwork outcome. Compare each route's warm medians with HTML ≤500 ms and usable content ≤1500 ms; no country-specific run is required.
- [ ] 5.5 Record separate Local/UAT/PRD outcomes and explain visits over 2500 ms. Keep repeatable application slowness and functional regressions open; retain evidence for isolated transport/tool outliers without promising universal latency. If the candidate remains slow, use existing request/SQL metrics to select the scoped follow-up from the design before adding instrumentation, batching or indexes. Archive only when the implementation and required acceptance have evidence.
