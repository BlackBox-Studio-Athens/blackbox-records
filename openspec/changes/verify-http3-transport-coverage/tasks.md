...
- [ ] 1.1 Run `pnpm openspec:guard`, reuse the accepted September 15 EmDash receipt and subsequent runtime-publication release evidence, then record current source/deployment/snapshot identities and reconcile affected contract differences; do not reopen accepted cutover or require deferred cleanup/archive.
- [ ] 1.2 Inventory current Local/UAT/PRD hostnames and every transport leg, including Pages gateway/PUBLIC_SITE/renderer/R2, combined CMS/commerce, public media and provider/webhook paths; verify ownership, native binding classification, and service-only renderer privacy.
...
- [ ] 2.2 Trace chosen asset/document/media/API reads and establish current account-wide Free-tier headroom with numeric caps; distinguish static delivery, renderer cache/pointer/R2 work and commerce object/D1 effects, excluding checkout/provider writes.
...
- [ ] 3.3 Run targeted diagnostic checks and strict OpenSpec validation; if implementation changes behavior, run `pnpm validate` against the final tree and applicable editor/publication checks from repository guidance. Record source fingerprints, results and unresolved hosted limitations; partial validation is not completion.
