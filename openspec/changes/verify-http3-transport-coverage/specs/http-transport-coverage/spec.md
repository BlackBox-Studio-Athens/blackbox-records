...
The system SHALL establish HTTP/3 coverage only after the EmDash operations epic completes accepted cutover, handoff, and baseline-spec reconciliation.

#### Scenario: EmDash remains unfinished

- **WHEN** transport implementation is considered before the epic is complete
- **THEN** only planning and safe bounded discovery proceed
- **AND** existing deployment, protocol settings, and runtime behavior remain unchanged.

#### Scenario: Coverage is reviewed

- **WHEN** the final Local, UAT, and PRD topology is inventoried
- **THEN** coverage includes public documents/assets/media, public API, protected staff/CMS/API, internal bindings, storage, external providers, and inbound webhooks
...
Transport evidence SHALL identify the target, time, protocol-capable client, negotiated protocol, cache/connection state, response status, timing samples, and applicable request/resource budget.
