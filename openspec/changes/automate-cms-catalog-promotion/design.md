...
The user requirement changes the target model: a site maintainer can create or update a buyable item in Decap and have the background system promote that item through UAT and production. UAT stays as close to production as possible, which means both environments need the same promotion state machine, the same artifact contract, the same drift classification, the same apply semantics, and comparable smoke evidence. The remaining differences are environment facts: Stripe test mode versus live mode, sandbox D1 versus production D1, sandbox Worker versus production Worker, and UAT site URL versus production site URL.

## Goals / Non-Goals

**Goals:**

- Make Decap-authored content the entry point for buyable catalog publication.
- Define a Desired Catalog State that is generated from CMS content and used by automation for both UAT and production.
- Support production catalog apply through the same reconciler/CLI/workflow shape as UAT.
...
- Add enough CMS validation that maintainers can publish live catalog items without editing scripts or provider dashboards.
- Automate generated artifact commits after CMS content changes so Decap commits do not leave `pnpm check` red.
...
### Desired Catalog State becomes the promotion input
...
- Desired availability policy: whether the variant should be published, withheld, or retired
- Desired stock seed policy for first publication only, not ongoing physical stock authority
- target environments: draft, UAT-only, or UAT-plus-production
- smoke candidate flags so workflows can pick a deterministic item for post-promotion proof

The artifact is generated from Astro/Decap content plus explicit CMS commerce fields. It is committed by automation and checked by `pnpm check`. The Worker does not read Astro content at runtime; it continues using committed backend artifacts and D1.

Alternative considered: keep deriving production prices from format labels. Rejected because format-based prices are acceptable sandbox defaults but too implicit for live commerce. Production needs explicit CMS-visible price fields or another explicit provider policy. Since the maintainer only uses the CMS, Decap must expose the required price inputs.

Alternative considered: make Stripe Dashboard the only place to enter production prices. Rejected because it breaks the maintainer-only-CMS goal and prevents UAT/prod parity.

### Desired Price is not checkout authority

Desired Price is the repo/CMS instruction for provisioning the provider catalog. Stripe active Price remains the authority used by checkout after apply.
...
This preserves the existing runtime rule while allowing CMS-driven automation:

```text
Decap content + commerce fields
...
Alternative considered: keep production as a separate hand-run checklist. Rejected because it makes UAT structurally unlike production and leaves CMS publication incomplete.

### Normal production promotion is automatic, not manually approved

The normal path after a CMS publish does not require a developer to click through Stripe, Wrangler, D1, or a manual GitHub approval gate. GitHub Actions environments can still scope secrets and concurrency, but required reviewers are not part of the default catalog-promotion path unless the repo owner later chooses a stricter release policy.
...
### Decap needs explicit commerce fields

Release and distro CMS collections need a commerce section that is visible and hard to misconfigure. Exact field names can be refined during implementation, but the model covers:

- `commerce.enabled`
- `commerce.publish_target`: `draft`, `uat`, or `uat_and_production`
- `commerce.price.amount_minor`
- `commerce.price.currency`
- `commerce.price.revision` or a derived hash used for Price replacement idempotency
- `commerce.tax_code`, defaulting to physical goods where appropriate
- `commerce.option_label`
- `commerce.stock.initial_online_quantity` for first publication only
- `commerce.smoke_candidate`
- `commerce.retire_at` or `commerce.retired` for planned removal from checkout

CMS previews show a compact commerce summary: target environments, generated slug/variant, desired price, tax code, stock seed, and whether production promotion will run after publish.

Alternative considered: infer all commerce data from title, format, and group. Rejected because production automation must not guess live payment-critical values.

### Bot commits own generated artifacts

After Decap writes content, a workflow runs artifact generation and creates a bot commit on the same branch if generated files drift. That commit must be the one used for provider apply and deployment. This keeps the repo source of truth complete without requiring the maintainer to run commands locally.
...
- provider apply waits for the artifact commit's repository gates, not the original CMS-only commit
...
The automation can seed initial D1 `Stock` and `ItemAvailability` rows for a new variant. After a variant exists in production, D1/operator stock remains authoritative. Later CMS edits must not overwrite production stock counts unless an explicit stock initialization or retirement operation is being performed.

UAT keeps large synthetic stock defaults for repeated testing. Production uses explicit CMS initial stock for first publication or keeps the item non-buyable until operator stock exists.
...
- [Risk] A maintainer enters the wrong production price in CMS. -> Mitigation: require explicit amount/currency fields, CMS preview summary, generated artifact diff, production smoke amount assertion, and fail-closed checkout revalidation. Optional later policy can add thresholds or reviewer rules for large changes without changing the core automation.
- [Risk] Bot artifact commits race with additional CMS edits. -> Mitigation: use branch/environment concurrency, rebase or rerun on latest branch head, and apply only from the commit whose artifacts and gates passed.
...
2. Add CMS commerce fields and validation for releases/distro.
...
4. Add bot artifact commit workflow for Decap content changes.
...
- For failed pre-apply runs: stop before mutation; maintainer fixes CMS content and republishes.
...
- For failed production after shopper exposure: disable checkout for the affected variant, preserve orders, inspect Promotion Evidence, and run a corrective promotion from a new CMS commit.

## Resolved Policy Points

- Production-targeted CMS changes use `uat_and_production`; production promotion never skips UAT proof for the same artifact commit.
