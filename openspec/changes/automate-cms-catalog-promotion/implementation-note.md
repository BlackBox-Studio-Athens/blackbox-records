...
- Both catalog promotion environments still need environment-scoped `STRIPE_PAYMENT_METHOD_CONFIGURATION_ID`, `CLOUDFLARE_API_TOKEN`, and `STRIPE_SECRET_KEY` before workflow proof can run. The secret names are not currently listed on either catalog promotion environment.
- A local sandbox runtime config probe found `STRIPE_SECRET_KEY`, `STRIPE_WEBHOOK_SECRET`, `CHECKOUT_RETURN_ORIGINS`, and `COMMERCE_DB` present through Worker configuration, but `STRIPE_PAYMENT_METHOD_CONFIGURATION_ID` missing. Local `pnpm stripe:webhooks:verify --env sandbox` could not run because no local `STRIPE_SECRET_KEY` process env was available.
- A local production runtime config probe failed before provider mutation because production environment configuration is not currently verifiable from this machine: `STRIPE_PAYMENT_METHOD_CONFIGURATION_ID`, `CHECKOUT_RETURN_ORIGINS`, and `COMMERCE_DB` reported missing; `STRIPE_SECRET_KEY` and `STRIPE_WEBHOOK_SECRET` reported unverified because Wrangler secret listing could not be read. `FLAGS` and `PRODUCTION_CATALOG_CRON` reported not applicable.
