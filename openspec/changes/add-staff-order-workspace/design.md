...
1. **Use the existing staff surface.** `/orders/` is implemented beside Content, Items and Stock in the combined CMS Worker. Preserve the current `/content/` landing and shared navigation, controls, internal transport and same-origin hosted API policy. A second admin application or detached staff deployment would duplicate Access and release work.
...
Reconciled 2026-09-17: all implementation tasks remain complete. Subsequent EmDash integration ships staff assets inside the combined CMS Worker through the paired code-release workflow; do not restore the retired detached staff pipeline. Existing hosted evidence remains historical evidence at its recorded revision, including the absence of populated PRD orders. Launch task 4.9 owns current protected Access/order inspection; populated paid-order proof comes from authorized test/live-smoke evidence, not fabricated PRD orders. Any future correction uses `pnpm validate` and `pnpm validate:editor`; compatible code rollback preserves order history and the accepted content snapshot.
