...
Initial hosted order acceptance was blocked: authenticated `GET /api/internal/orders?limit=100` returned 500 with `no-store`. Read-only PRD D1 inspection found migrations applied only through 0008, missing order expiry, fulfillment and monetary columns, and no `PaidOrderDelivery` table. Orders and order lines were both empty. Migrations 0009–0018 were pending; this was production schema lag, not a staff-login failure. The static deployment workflow does not apply production D1 migrations.

## Production database migration and completed hosted checks

The user explicitly approved one production apply on 2026-09-11. `pnpm --filter @blackbox/backend d1:migrations:apply:prd` successfully applied the reviewed migrations 0009–0018 from release `7bfb062f`. They update catalog snapshot/event bookkeeping, order expiry and fulfillment fields, the notification outbox, stock revision validation, review reasons and monetary snapshots. Rechecked preconditions found no matching canonical or duplicate Caregivers rows, no orders or order lines, and no invalid stock rows.

A local SQLite upgrade rehearsal from schema 0008 through 0018 passed, preserving representative stock quantities, retaining zero orders and passing foreign-key checks. The refreshed pre-apply D1 Time Travel bookmark is `0000000b-00000000-000050e3-62feb02290d4d021a03782e29637ed4a`. Post-apply migration listing reports none pending; `PRAGMA foreign_key_check` returned no violations. Orders and deliveries remain empty, and existing physical/online stock quantities are unchanged.

Native Browser Use repeated the protected Stock refresh successfully, then verified Orders displays its empty recent subset with a successful read time. Both all-status and Paid-filter order GETs returned 200 with `no-store`; a deliberately missing session lookup returned 404 with the expected guidance, and Back returned to the list. Observed internal requests were same-origin GETs only. Existing Access allow/deny evidence above remains applicable without any auth changes.

Hosted deployment, Access, empty-list/filter, missing-session and Stock checks are complete. Production has zero orders, so populated-order detail coverage remains the completed local synthetic checks, not a claim of production detail inspection. No fake PRD orders, order-payload evidence, payment, refund, resend, customer communication or dispatch action was created. Checkout launch remains unapproved by this work.
