...
The system SHALL start paid-order email notification side effects only after verified Stripe webhook reconciliation applies the first paid CheckoutOrder transition.
...
- **THEN** paid-order email notification work starts after order state and stock reconciliation succeed.
...
- **THEN** no paid-order email notification is started.
