...
- [ ] 4.7 After accepting `align-cloudflare-uat-and-release-promotion`, explicitly promote that accepted UAT candidate SHA/run's verified Worker and PRD-targeted static artifacts to technical PRD origins with separate code confirmation; verify final canonical metadata, reachable catalog/email images while the apex still serves Holding Page, the technical/apex return allowlist, catalog, webhook, D1, Access, Cron, and configuration with checkout closed. Main pushes and live-catalog confirmation do not authorize this code promotion.
