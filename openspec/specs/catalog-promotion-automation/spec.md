...
Defines revision-bound software promotion and explicit catalog recovery while preserving runtime catalog authority and independent PRD launch controls.

## Requirements

### Requirement: Release builds use one source revision

Each Software Release SHALL use one source SHA and explicit target content snapshot. Authoritative catalog records SHALL be runtime D1 data and SHALL NOT require artifact bot commits. Repository migration data SHALL be loaded only by explicit recovery tooling and Local/UAT fixtures, never as runtime fallback or routine deployment input.
