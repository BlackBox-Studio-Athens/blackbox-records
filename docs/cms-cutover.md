...
### Approved catalog linkage and publication setup — completed

The user approved this batch. Apply run `34945947353` succeeded: Disintegration is linked to CMS record `01M2J1HS3EKT7PXS24JDEWJ7WB`, and the existing live Stripe key was installed directly in the PRD Worker. The native read-only export token and PRD-only Access service token expire September 15, 2027. GitHub repository secrets hold export/Access/completion credentials; the Worker holds the matching completion credential and an Actions read/write, Metadata read-only fine-grained token limited to this repository, with no expiration as requested. The existing Google member policy remains in place.

All 128 other editorial records are published with source-value comparisons, and Disintegration's supported item command returned publication `27e3d7d7-3c8c-4844-b197-f0a0b2385127`. PRD lacked a scheduled trigger, so the existing dispatcher could not run. A five-minute trigger now invokes the existing combined handler (288 scheduled invocations/day), with no new Worker or service. Publication run `34948274868` started from that trigger. That first attempt failed before deployment; the successful retry and final receipt are recorded below.

Evidence: `prd-approved-publication-setup-mcp.json`, `prd-editorial-publication-batch.json`, `prd-item-publication-pilot.json`, `prd-publication-schedule.json`. The ten focused release-ordering tests and strict OpenSpec validation pass in `prd-final-focused-mcp.json`; full-suite repeats are intentionally skipped under the user's revised acceptance scope.

The first publication captured 405 bounded CMS reads but correctly stopped before deployment: PRD's one runtime item excluded the other published distro display entries, and the existing image/output check rejected the incomplete storefront. Fix `123b905a` preserves published unbound distro display identities in the snapshot without creating commerce records or changing bound identities. All 13 focused snapshot checks pass. The confirmed failed run was reconciled through the existing journal function, then the same item operation requested retry publication `af0bc3fa-b62b-4030-baae-bcd29faeda46`, run `34949495942`. This reuses the deployed public code, keeps the captured content/stock/provider state intact, and supplies actual failed-publication recovery evidence rather than another artificial rehearsal.
...
The reviewed approval scope was the one live linkage and these PRD credentials. It has been consumed by the successful batch above; do not repeat apply or recreate credentials. Values remain in GitHub/Worker secrets, never chat, public build variables or committed files.

The approved setup and first immutable PRD snapshot publication are complete with the already deployed source SHA. Use the existing 600-request capture ceiling proven by UAT, preserve the existing price/stock, and keep both shopper launch gates disabled. Identity reconciliation passed before releasing the editorial write freeze. Hosted Sveltia is disabled; dormant sources and generation hooks remain in the deferred cleanup inventory. This is not authorization to promote a future unreviewed code artifact.
...
### Final acceptance — Live, September 15, 2026

Publication run [34949495942](https://github.com/BlackBox-Studio-Athens/blackbox-records/actions/runs/34949495942) succeeded. Publication `af0bc3fa-b62b-4030-baae-bcd29faeda46` is Live with snapshot SHA-256 `6289a67eb4aed84f5de0b0b85a0db0966040650a2d0e7b842a422841e218cb96`. Public code remains `af93b371e4eaca0b1b418b62c59da86ad8e36f8c`; this content run did not promote newer software or deploy the Worker.

Read-only reconciliation at `2026-09-15T09:02:28Z` passed: all 101 imported distro titles appear among 102 searchable Store Items; nine detail pages, nine overlays, sitemap and metadata match the imported content. The initial helper expected the larger UAT fixture count of 104; it was corrected to the actual frozen PRD import count, with every imported distro title still checked. Stock, Price bindings and order history match the pre-import snapshot exactly. Disintegration remains EUR 28.00 inclusive, 15 physical / 12 online; native checkout remains disabled. Hosted `/admin/config.yml` has no writable GitHub backend. Protected staff already passed Chrome Blackbox acceptance on the combined Worker; the authenticated publication endpoint now reports Live. Editorial writes may resume in EmDash.

Evidence: `prd-publication-public-surfaces.json`, `prd-publication-final-reconciliation.json`, `prd-final-reconciliation-mcp.json`, plus the successful CI receipt. Reused accepted combined candidate `34937507986` for full unit/check/build, CMS/KV rejection and prior Local/UAT browser/provider/recovery evidence. Later capture changes passed 13 focused snapshot checks; release ordering passed ten checks. Full-suite repeats and another browser/rollback matrix were skipped at the user's request. Two bounded captures used 405 CMS reads each; final public reconciliation used 22 reads. The existing free-tier batch budget remains sufficient. Physical legacy-file and inactive staff Pages branch deletion is deferred in [the retirement inventory](cms-retirement.md), not claimed complete.

### Approved batch scope and preflight (historical)
