...
The apply path requires both live-catalog and CMS-cutover confirmation inputs plus the exact hash, with code promotion false. One-run authorization remains pending. Backend type-checking, lint and 11 focused tests passed through WebStorm. Full UAT runtime acceptance remains bound to the preceding successful candidate; this command is migration tooling outside runtime/build entrypoints. Evidence: `.codex-artifacts/emdash-m1/prd-initial-price-plan-34940044370.log` and `prd-initial-price-checks.json`.
