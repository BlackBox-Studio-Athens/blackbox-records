...
3. Remove the obsolete production control variable from GitHub and Worker configuration. Do not replace it with a persistent catalog-preparation variable; select `confirm_live_catalog_changes=true` only for an authorized exact-commit promotion run.
4. Keep `PRD_LAUNCH_APPROVED` absent until the sole launch approver accepts final evidence. At launch, checkout still requires `native_checkout_enabled` to resolve true.
