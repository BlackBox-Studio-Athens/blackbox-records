...
The implementation uses [Wrangler's native D1 exports](https://developers.cloudflare.com/workers/wrangler/commands/d1/) and Node's built-in SQLite reader. Recovery creates tables before inserting data through bound D1 statements, avoiding both foreign-key ordering and D1's SQL-literal size limit. Foreign keys remain enforced in the destination. The single recovery transaction is capped at 10,000 statements; larger sites need a reviewed dependency-aware batching strategy. Hosted bucket provisioning, schedule activation and the full recovery rehearsal remain explicit acceptance work until recorded evidence proves them.
