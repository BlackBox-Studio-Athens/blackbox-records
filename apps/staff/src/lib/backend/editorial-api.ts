...
export type EditorialMedia = { id: string; filename: string; url: string; alt: string | null };
...
export async function editorialRequest<T>(base: string, path: string, body?: object | FormData): Promise<T> {
  ...
  if (!response.ok)
    throw new EditorialApiError(
      response.status,
      response.status === 409
        ? 'This record already exists or has changed. Choose the existing record.'
        : 'We could not confirm this request. Check your connection and try again.',
    );
