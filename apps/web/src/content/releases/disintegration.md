---
title: Disintegration
artist: afterwise
release_date: 2026-06-09
cover_image: 656856327_18427527979186423_8617747121554203403_n.jpg
cover_image_alt: Disintegration by Afterwise
merch_url: /store/
bandcamp_embed_url: https://bandcamp.com/EmbeddedPlayer/album=3481803854/size=large/bgcol=0d0d0d/linkcol=f5f5f5/artwork=big/transparent=true/
tidal_url: https://tidal.com/album/521945607
summary: >-
  Afterwise's six-track debut album blends instrumental post-rock atmosphere
  with post-metal weight. Written and performed by the Athens band, it was
  recorded and mixed at BlackBox Studio and released by BlackBox Records.
  Digital availability is confirmed by Bandcamp; Black Vinyl LP and CD are
  label editions.
formats:
  - Black Vinyl LP
  - CD
  - Digital
credits:
  - role: Music written and performed by
    name: Afterwise
  - role: Piano / Synth
    name: George Stamatiou
  - role: Guitar
    name: Stavros Apostolou
  - role: Guitar
    name: Ilias Daramouskas
  - role: Bass
    name: Markos Kousounadis Knousen
  - role: Drums
    name: Giannis Avraam
  - role: Recorded and mixed by
    name: Jim Spanos
  - role: Recorded and mixed at
    name: BlackBox Studio, Athens, Greece
  - role: Mastered by
    name: Nikos Dimitrakakos and Jim Spanos
  - role: Mastered at
    name: Unreal Studio, Athens, Greece
  - role: Artwork by
    name: Joshua Takak
  - role: Released by
    name: BlackBox Records
---
