...
describe('Distro Coverflow progressive enhancement', () => {
  it('derives full-group position state while retaining one complete card render path', () => {
    expect(pageSource).toContain('coverflowEligible: totalCount > DISTRO_COVERFLOW_PREVIEW_SIZE');
    expect(pageSource).toContain('const totalCount = group.entries.length');
    expect(pageSource).toContain('const positionedCount = Math.min(DISTRO_COVERFLOW_PREVIEW_SIZE, totalCount)');
    ...
    expect(pageSource).toContain('data-distro-coverflow-preview-count');
    expect(pageSource).toContain('data-distro-coverflow-remaining-count');
    expect(pageSource).toContain('data-distro-coverflow-initial-position-ratio');
    expect(pageSource).toContain('--distro-coverflow-position-ratio: ${group.initialPositionRatio}');
    ...
    expect(pageSource).toContain('data-distro-coverflow-current-value');
    expect(pageSource).toContain('data-distro-coverflow-remaining-value');
    expect(pageSource).toContain('data-distro-coverflow-summary');
    expect(pageSource).toContain('class="distro-coverflow-rail" aria-hidden="true"');
    expect(pageSource).toContain('data-distro-coverflow-disclosure-rail');
    expect(pageSource).toContain('class="distro-coverflow-actions__toggle"');
  ...
  it('uses one responsive six-position stage across every card with a flat reduced-motion fallback', () => {
    ...
    expect(searchSource).toContain("group.element.setAttribute('aria-roledescription', 'carousel')");
    expect(searchSource).toContain("group.element.removeAttribute('aria-roledescription')");
    ...
    expect(cssSource).toContain('--distro-cover-size: clamp(13.5rem, 20vw, 16rem)');
    expect(cssSource).toContain('--distro-cover-near-shift: clamp(7.5rem, 16vw, 11rem)');
    ...
    expect(cssSource).toMatch(/data-distro-coverflow-ready[\s\S]*?\.distro-group-chunk[\s\S]*?display: contents/);
    expect(cssSource).toMatch(
      /data-distro-coverflow-ready[\s\S]*?data-distro-coverflow-card\]:not\(\[data-distro-coverflow-position\]\)[\s\S]*?display: none/,
    );
    expect(cssSource).toMatch(/\.distro-card__content[\s\S]*?display: none/);
    expect(cssSource).toContain('animation: distro-catalog-reveal 300ms');
    expect(cssSource).toContain('animation: distro-coverflow-preview-rail-in 360ms');
    expect(cssSource).toContain('animation: distro-coverflow-disclosure-fill 180ms');
    expect(cssSource).toContain('transform: scaleX(var(--distro-coverflow-position-ratio))');
    expect(cssSource).toContain('transition: transform 260ms cubic-bezier(0.22, 1, 0.36, 1)');
    expect(cssSource).toMatch(/\.distro-coverflow-stat[\s\S]*?justify-content: flex-end/);
    expect(cssSource).toContain("data-distro-coverflow-reveal='catalog-pending'");
    ...
    expect(cssSource).toMatch(
      /prefers-reduced-motion: reduce[\s\S]*?distro-coverflow-rail__fill[\s\S]*?animation: none/,
    );
    expect(cssSource).toMatch(
      /prefers-reduced-motion: reduce[\s\S]*?distro-coverflow-rail__fill[\s\S]*?transition: none/,
    );
  });
});
