...
const querySchema = z
  .object({
    view: z.enum(['changes', 'overview']).optional(),
...
export async function readStaffWorkspace(
  ...
  const overview = query.data.view === 'overview';
  if (overview && [...new URL(request.url).searchParams.keys()].some((key) => key !== 'view'))
    return problem(400, 'Overview does not accept selection, paging or filters.');
  ...
  async function readPending(
    pages: { section: string; items: Awaited<ReturnType<ContentRepository['findMany']>>['items'] }[],
  ) {
    const selectedEntries = JSON.stringify(
      pages.flatMap((page) =>
        page.items.map((item) => ({
    ...
    WHERE p.environment = ? AND p.status = 'pending' AND (
      ...
      .bind(deps.environment, selectedEntries, selectedEntries)
      .all<{ id: string; requestedRevision: string; requestJson: string | null }>();
  }
  async function readOverview() {
    const repository = new ContentRepository(deps.runtime.db);
    const pages = await Promise.all(
      Object.keys(sourceCollectionNames).map(async (section) => ({
        section,
        ...(await repository.findMany(section, { limit: 5, orderBy: { field: 'updatedAt', direction: 'desc' } })),
      })),
    );
