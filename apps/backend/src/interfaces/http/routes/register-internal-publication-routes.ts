  ...
  app.openapi(
    ...
    async (context) => {
      ...
      try {
        ...
        const item = await catalog.findByVariantId(parseVariantId(variantId));
        ...
        const actions =
          detail.publicationStatus === 'live'
            ? undefined
            : [
                apiAction({
                  href: apiPath('api', 'internal', 'variants', variantId, 'publication'),
                  method: 'POST',
                  operationRef: 'publishCatalogItem',
                  parameters: {
                    body: {
                      ...(detail.pending ? { operationId: detail.pending.operationId } : {}),
        ...
        return jsonNoStore(context.json(addHypermedia(detail, publicationLinks(variantId), actions), 200));
      } catch (error) {
  ...
  app.openapi(
    ...
    async (context) => {
      ...
      try {
        ...
        return jsonNoStore(context.json(addHypermedia(resultSchema.parse(result), publicationLinks(variantId)), 200));
      } catch (error) {
...
function publicationLinks(variantId: string) {
  return [
    apiLink({
      href: apiPath('api', 'internal', 'variants', variantId, 'publication'),
      rel: 'self',
