...
export function createStripeSandboxSmokeScenarios(
  ...
  function requireCatalogProjectionEntry(
    entry: StripeCatalogStoreItemContract | undefined,
  ...
  const STRIPE_SANDBOX_SMOKE_SCENARIOS: Record<StripeSandboxSmokeScenarioName, StripeSandboxSmokeScenario> = {
    checkout_surface: {
  ...
  const fixedSnapshot = createSmokeStoreCartLineItemSnapshot(requireFixedSmokeCatalogProjectionEntry());
  for (const scenario of Object.values(STRIPE_SANDBOX_SMOKE_SCENARIOS)) scenario.lineItemSnapshot ??= fixedSnapshot;
  return STRIPE_SANDBOX_SMOKE_SCENARIOS;
}

export const STRIPE_TEST_CARD_DOCS_URL = 'https://docs.stripe.com/testing#cards';
type SmokeCatalogProjectionEntry = StripeCatalogStoreItemContract & {
  expectedSandboxPrice: NonNullable<StripeCatalogStoreItemContract['expectedSandboxPrice']>;
};

export function resolveSelectedStripeSandboxScenarios(
  selection: StripeSandboxSmokeScenarioSelection,
  scenarios: Record<StripeSandboxSmokeScenarioName, StripeSandboxSmokeScenario>,
): StripeSandboxSmokeScenario[] {
  if (typeof selection !== 'string') {
    return selection.map((name) => scenarios[name]);
  }

  return resolveSmokeScenarioSelection(selection, Object.values(scenarios));
}
...
function createSmokeStoreCartLineItemSnapshot(entry: SmokeCatalogProjectionEntry): CartLineItemSnapshot {
